...
(function (global) {
  ...
  Report.askName = function (done) {
    ...
    el('p', 'muted', body, '비워 두면 손으로 쓸 수 있게 빈칸으로 인쇄된다.');
  ...
  Report.buildReport = function () {
    ...
    el('p', 'sheet-note', page,
      '길이는 원본 지도 이미지의 픽셀 단위이며, 등급은 근사값이다.');
    ...
    el('p', 'sheet-p', page,
      '종이에 그린 지도는 별이 어디에 있는지를 알려주지만 별의 크기가 모두 같아 밝기는 알 수 없다. ' +
      '돌에 새긴 지도는 별마다 크기를 다르게 새겼고, 그 크기는 실제 밝기와 맞아떨어진다. ' +
      '조선의 천문학자들은 위치뿐 아니라 밝기까지 관측해 기록으로 남긴 것이다.');
  ...
  Report.printSheet = function () {
    ...
    el('p', 'sheet-sub', head, '인쇄한 지도에서 자로 재어 적어 보자');
    ...
    sec(page, '그래프 그리기');
    el('p', 'sheet-p', page,
      '가로는 별의 밝기(왼쪽이 밝은 별), 세로는 내가 잰 크기. 별 하나에 점 하나를 찍어 보자.');
